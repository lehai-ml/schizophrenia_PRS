...
###diffusion data
for i in tqdm.tqdm(range(11)):
    X_preterm=np.load('./notebooks_results/dHCP/preprocessed_data/diffusion/preterm/high_low_risk_diffusion_preterm_dataset_PRS%d.npy'%i)
    y_preterm=np.load('./notebooks_results/dHCP/preprocessed_data/diffusion/preterm/high_low_risk_diffusion_preterm_dataset_PRS%d_label.npy'%i)

    X_term=np.load('./notebooks_results/dHCP/preprocessed_data/diffusion/term/high_low_risk_diffusion_term_dataset_PRS%d.npy'%i)
    y_term=np.load('./notebooks_results/dHCP/preprocessed_data/diffusion/term/high_low_risk_diffusion_term_dataset_PRS%d_label.npy'%i)

    X_mean_connectivity_high_risk_preterm=np.mean(X_preterm[y_preterm==1],axis=0)
    X_mean_connectivity_low_risk_preterm=np.mean(X_preterm[y_preterm==0],axis=0)
    
    X_mean_connectivity_high_risk_term=np.mean(X_term[y_term==1],axis=0)
    X_mean_connectivity_low_risk_term=np.mean(X_term[y_term==0],axis=0)

    X_mean_connectivity_high_risk_preterm=data_preprocessing.reverse_lower_triangle(X_mean_connectivity_high_risk_preterm)
    X_mean_connectivity_low_risk_preterm=data_preprocessing.reverse_lower_triangle(X_mean_connectivity_low_risk_preterm)
    
    X_mean_connectivity_high_risk_term=data_preprocessing.reverse_lower_triangle(X_mean_connectivity_high_risk_term)
    X_mean_connectivity_low_risk_term=data_preprocessing.reverse_lower_triangle(X_mean_connectivity_low_risk_term)

    #generate binarized matrices for over a range of network sparsity for high and low risk groups
    binarized_low_risk_matrices_preterm=np.asarray([data_preprocessing.lower_triangle(graph_analysis.binarize_matrix_based_on_sparsity_threshold(X_mean_connectivity_low_risk_preterm,network_sparsity_threshold/100,bins=100)) for network_sparsity_threshold in np.linspace(1,30,30)])
    
    binarized_high_risk_matrices_preterm=np.asarray([data_preprocessing.lower_triangle(graph_analysis.binarize_matrix_based_on_sparsity_threshold(X_mean_connectivity_high_risk_preterm,network_sparsity_threshold/100,bins=100)) for network_sparsity_threshold in np.linspace(1,30,30)])


    binarized_low_risk_matrices_term=np.asarray([data_preprocessing.lower_triangle(graph_analysis.binarize_matrix_based_on_sparsity_threshold(X_mean_connectivity_low_risk_term,network_sparsity_threshold/100,bins=100)) for network_sparsity_threshold in np.linspace(1,30,30)])
    binarized_high_risk_matrices_term=np.asarray([data_preprocessing.lower_triangle(graph_analysis.binarize_matrix_based_on_sparsity_threshold(X_mean_connectivity_high_risk_term,network_sparsity_threshold/100,bins=100)) for network_sparsity_threshold in np.linspace(1,30,30)])
    
    #calculating graph_metrics_low_risk

    for threshold in tqdm.tqdm(range(30)):
        temp_path_low_risk='./notebooks_results/dHCP/log/diffusion/preterm/PRS%d/obsv_graph_metrics_diffusion_low_risk_preterm_PRS%d.txt'%(i,i)
        low_risk_metrics=graph_analysis.calculate_network_metrics_random_volumetric_data(data_preprocessing.reverse_lower_triangle(binarized_low_risk_matrices_preterm[threshold],51))

        with open(temp_path_low_risk,'ab') as f:
            np.savetxt(f,np.asarray([low_risk_metrics]),delimiter='\t')

        temp_path_high_risk='./notebooks_results/dHCP/log/diffusion/preterm/PRS%d/obsv_graph_metrics_diffusion_high_risk_preterm_PRS%d.txt'%(i,i)
        high_risk_metrics=graph_analysis.calculate_network_metrics_random_volumetric_data(data_preprocessing.reverse_lower_triangle(binarized_high_risk_matrices_preterm[threshold],51))
    ...
    for threshold in tqdm.tqdm(range(30)):
        temp_path_low_risk='./notebooks_results/dHCP/log/diffusion/term/PRS%d/obsv_graph_metrics_diffusion_low_risk_term_PRS%d.txt'%(i,i)
        low_risk_metrics=graph_analysis.calculate_network_metrics_random_volumetric_data(data_preprocessing.reverse_lower_triangle(binarized_low_risk_matrices_term[threshold],51))

        with open(temp_path_low_risk,'ab') as f:
            np.savetxt(f,np.asarray([low_risk_metrics]),delimiter='\t')

        temp_path_high_risk='./notebooks_results/dHCP/log/diffusion/term/PRS%d/obsv_graph_metrics_diffusion_high_risk_term_PRS%d.txt'%(i,i)
        high_risk_metrics=graph_analysis.calculate_network_metrics_random_volumetric_data(data_preprocessing.reverse_lower_triangle(binarized_high_risk_matrices_term[threshold],51))
