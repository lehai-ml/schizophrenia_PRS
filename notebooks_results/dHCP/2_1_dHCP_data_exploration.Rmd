...
```{r}
european_diffusion_dataset.term<-read.csv('./preprocessed_data/diffusion/european_ID_diffusion_dataset_term_without_outliers.csv')
```
...
```{r}
df.allPRS_nointeraction<-data.frame(Connection=character(),
                        prs_pval=double(),
                        prs_coef=double(),
                        GA_pval=double(),
                        GA_coef=double(),
                        ...
                        Adj_R_square=double(),
                        model_pval=double(),
                        PRS_threshold=character(),stringsAsFactors = FALSE)
...
df.allPRS_nointeraction=read.csv('./observed_univariate_test.csv')
head(df.allPRS_nointeraction)
```

Here the best prs_pval is 0.0001254505, for IFGtriang.R_ORBmid.R at PRS threshold of 0.5.
...
```{r}
diffusion.pca<-prcomp(european_diffusion_dataset.term.removed_zeros[6:2518])
...
```{r}
View(summary(diffusion.pca)$rotation[,'PC1'])
```


```{r}
original_subset.pca=cbind(european_diffusion_dataset.term.removed_zeros[,4:6],diffusion.pca$x[,1:89],european_diffusion_dataset.term.removed_zeros[,2712:2722])# this retains only the diffusion PCA data and PRS scores.
...
```{r}
df.allPRS_nointeraction.PCA<-data.frame(Connection=character(),
                        prs_pval=double(),
                        prs_coef=double(),
                        GA_pval=double(),
                        GA_coef=double(),
